...
def write_database(data, cursor):
    for index in range(0, len(data)):
        ...
        try:
            if card[10] != 'HERO_SKINS' and card[5] != 'HERO':
...
def create_database(connection):
    try:
        connection.cursor().execute('''CREATE TABLE cards (
                    ...
                    collectible BOOLEAN,
                    cardSet TEXT,
                    faction TEXT COLLATE NOCASE,
                    artist TEXT,
                    flavor TEXT,
                    mechanics TEXT COLLATE NOCASE,
                    dust TEXT,
                    race TEXT COLLATE NOCASE,
                    playerClass TEXT COLLATE NOCASE,
                    howToEarn TEXT,
